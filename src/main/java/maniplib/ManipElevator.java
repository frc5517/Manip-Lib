...
public class ManipElevator extends SubsystemBase {
    ...
    public ManipElevator(ManipMotor motor, ManipElevatorConfig config) {
        ...
        if (motor instanceof ManipSparkMax) {
            ...
            ((SparkMax) motor.getMotor()).configure(sparkConfig, SparkBase.ResetMode.kNoResetSafeParameters, SparkBase.PersistMode.kPersistParameters);
    ...
    public void reachGoal(double goal) {
        if (motor instanceof ManipSparkMax) {
            elevatorLimitSwitchFunction();
            ((SparkClosedLoopController) controller).setReference(goal,
    ...
    public Trigger atHeight(double height, double tolerance) {
        return new Trigger(
                () -> MathUtil.isNear(height, encoder.getPosition(), tolerance));
    }

    public Command setGoal(double goal) {
        return runEnd(() -> reachGoal(goal),
        () -> {
            stopCommand();
        });
    }
